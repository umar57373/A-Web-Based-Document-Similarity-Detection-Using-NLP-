...
import { FileSearch, Mail, Lock, Eye, EyeOff, AlertCircle, Chrome } from "lucide-react";
      ...
      >
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="max-w-md text-center">
          ...
          <p className="text-lg leading-relaxed" style={{ color: "hsl(220, 15%, 72%)" }}>
            AI-powered document similarity detection for academic integrity. Upload assignments and instantly check for similarities.
          </p>
          <div className="mt-12 grid grid-cols-3 gap-4">
            {[{ value: "99%", label: "Accuracy" }, { value: "< 5s", label: "Analysis" }, { value: "256-bit", label: "Encrypted" }].map(stat => (
              <div key={stat.label} className="rounded-xl p-3" style={{ background: "hsl(222, 28%, 16%)", border: "1px solid hsl(222, 22%, 22%)" }}>
                <div className="text-xl font-bold" style={{ color: "hsl(168, 60%, 55%)" }}>{stat.value}</div>
                <div className="text-xs mt-1" style={{ color: "hsl(220, 15%, 58%)" }}>{stat.label}</div>
              </div>
            ))}
          </div>
        </motion.div>
          ...
          {!showForgot ? (
            <>
              ...
              <Button
                type="button"
                variant="outline"
                className="w-full mb-4 font-medium gap-2"
                size="lg"
              ...
              <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                ...
                <Button type="submit" className="w-full font-semibold" size="lg" disabled={isLoading}>
                  {isLoading ? (
                    <><span className="h-4 w-4 border-2 border-current border-t-transparent rounded-full animate-spin mr-2" />Signing in...</>
                  ) : "Sign in"}
                </Button>
              </form>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
              <button onClick={() => setShowForgot(false)} className="text-sm text-muted-foreground hover:text-foreground mb-6 flex items-center gap-1">
                ← Back to sign in
              </button>
              ...
              <form onSubmit={handleForgotPassword} className="space-y-4">
                ...
                <Button type="submit" className="w-full font-semibold" size="lg" disabled={isForgotLoading}>
